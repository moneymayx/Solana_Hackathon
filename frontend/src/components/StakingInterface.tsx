...
export default function StakingInterface({ userId, walletAddress, currentBalance = 0 }: StakingInterfaceProps) {
  ...
  const fetchStakingData = async () => {
    try {
      ...
      const positionsData = await tokenAPI.getStakingPositions(userId)
      ...
      const revenueStats = await tokenAPI.getPlatformRevenue()
  ...
  const handleStake = async () => {
    ...
    setStaking(true)
    try {
      // Call the stake API endpoint directly
      const response = await fetch('http://localhost:8000/api/token/stake', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          transaction_signature: null // Will be set by backend in mock mode or generated by frontend
        })
      })

      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}))
        throw new Error(errorData.detail || errorData.error || 'Failed to stake tokens')
      }
