...
export default function BountyCard({ bounty, className }: BountyCardProps) {
  ...
  return (
    <div
      className={cn(
        "group relative bg-white border-2 rounded-xl p-6 transition-all duration-300 hover:scale-105",
      ...
      style={{
        ...
        boxShadow: isHovered ? `0 25px 50px -12px ${colors.primary}40` : `0 10px 25px -3px ${colors.primary}20`
      }}
    ...
    >
        ...
        <div className="flex items-center justify-between space-x-3 mt-auto">
          ...
          <Link href={`/bounty/${bounty.id}?action=beat`} className="flex-1">
            <button 
              className="w-full px-4 py-3 rounded-lg text-sm font-semibold transition-all duration-200 text-white border-2 shadow-md hover:shadow-lg"
              style={{
